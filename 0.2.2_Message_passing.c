...
void moveAgents(Data data, int *moves, int n_moves, int rank, int wd_size) {
    ...
    for (int i = 0; i < wd_size; i++) {
        for (int j = 0; j < n_moves; j++) {
            if (calcMembership(data, i, data.my_emp_slots[j], wd_size)) {
                ...
                if (i == rank) lc_moves[all_msg[i].n_agent] = moves[j] - data.sec_disp[rank];
                all_msg[i].n_agent++;
            }
        }
    }

    if (all_msg[rank].n_agent == 0) free(lc_moves);
    ...
    for (int i = 0; i < wd_size; i++) {
        ...
        if (i == rank && all_msg[i].n_agent != 0) lc_moves = realloc(lc_moves, sizeof(int) * all_msg[i].n_agent);
    }
    ...
    for (int i = 0; i < wd_size; i++) {
        if (i != rank) {
            ...
            if (all_msg[i].n_agent != 0) {
                ...
                printVetInt(lc_moves, all_msg[i].n_agent);
            } else {
