...
void generateMat(char *i_mat, int ROWS, int COLUMNS) {
...
void printMat(char *i_mat, int ROWS, int COLUMNS) {
...
void printResultHTML(char *i_mat, char *r_mat, int n_itc, double t, int wd_size, int ROWS, int COLUMNS) {
...
void calcSizes(int wd_size, Data data, int ROWS, int COLUMNS) {
...
int calcStart(int rank, int wd_size, int ROWS, int COLUMNS) {
...
int calcFinish(Data data, int rank, int wd_size, int ROWS, int COLUMNS) {
...
void convertIndex(int id, int *rowS_index, int *col_index, int ROWS, int COLUMNS) {
...
void satCorner(int id, Data data, int *neigh, int *my_kind, int pos, int ROWS, int COLUMNS) {
...
void satEdge(int id, Data data, int *neigh, int *my_kind, int pos, int ROWS, int COLUMNS) {
...
void satCenter(int id, Data data, int *neigh, int *my_kind, int ROWS, int COLUMNS) {
...
int calcSat(int id, Data data, int rank, int ROWS, int COLUMNS) {
    //Calculate the satisfaction of an agent
    int neigh = 0, my_kind = 0;
    int rowS = data.sec_size[rank] / COLUMNS;
    int rowS_index, col_index;

    convertIndex(id, &rowS_index, &col_index, ROWS, COLUMNS);

    if (rowS_index == 0)
        if (col_index == 0)
            satCorner(id, data, &neigh, &my_kind, 0, ROWS, COLUMNS);  //Upper left corner
        else if (col_index == COLUMNS - 1)
            satCorner(id, data, &neigh, &my_kind, 1, ROWS, COLUMNS);  //Upper right corner
        else
            satEdge(id, data, &neigh, &my_kind, 0, ROWS, COLUMNS);  //Upper edge

    else if (rowS_index == rowS - 1)
        if (col_index == 0)
            satCorner(id, data, &neigh, &my_kind, 2, ROWS, COLUMNS);  //Lower left corner
        else if (col_index == COLUMNS - 1)
            satCorner(id, data, &neigh, &my_kind, 3, ROWS, COLUMNS);  //Lower right corner
        else
            satEdge(id, data, &neigh, &my_kind, 3, ROWS, COLUMNS);  //Lower edge

    else if (col_index == 0 && rowS_index > 0 && rowS_index < rowS - 1)
        satEdge(id, data, &neigh, &my_kind, 1, ROWS, COLUMNS);  //Left edge

    else if (col_index == COLUMNS - 1 && rowS_index > 0 && rowS_index < rowS)
        satEdge(id, data, &neigh, &my_kind, 2, ROWS, COLUMNS);  //Right edge
    else
        satCenter(id, data, &neigh, &my_kind, ROWS, COLUMNS);  //Center
...
int findMoves(Data data, Move *my_moves, int rank, int wd_size, int ROWS, int COLUMNS) {
    ...
    for (int i = data.r_start; i <= data.r_finish; i++) {
        if (data.sub_mat[i] != ' ')
            if (!calcSat(i, data, rank, ROWS, COLUMNS)) {
                if (n_moves == 0) break;
...
void main(int argc, char *argv[]) {
    ...
    if (wd_size <= ROWS) {
        ...
        if (rank == MASTER) {
            ...
            generateMat(i_mat, ROWS, COLUMNS);
            //sampleMat(i_mat);
        }
        ...
        calcSizes(wd_size, data, ROWS, COLUMNS);
        ...
        data.r_start = calcStart(rank, wd_size, ROWS, COLUMNS);
        data.r_finish = calcFinish(data, rank, wd_size, ROWS, COLUMNS);
        ...
        while (n_itc != 0) {
            ...
            if (findMoves(data, my_moves, rank, wd_size, ROWS, COLUMNS)) break;
    ...
    if (wd_size <= ROWS) {
        ...
        if (rank == MASTER) {
            ...
            if (OUTPUT_TYPE == 0) {
                printResultHTML(i_mat, r_mat, N_ITERACTION - n_itc, end - start, wd_size, ROWS, COLUMNS);
            } else {
                ...
                if (OUTPUT_TYPE == 1) {
                    ...
                    printMat(r_mat, ROWS, COLUMNS);
                    printf("And this is the initial matrix, bye-bye 👋: \n");
                    printMat(i_mat, ROWS, COLUMNS);
                }
